---
{"publish":true,"permalink":"/Theme for 2026.md","title":"The Year of \"Out\"","created":"2025-12-25 13:17:42","modified":"2026-01-01 23:26:50","published":"2026-01-01T23:26:50.941+00:00","cssclasses":""}
---


# The Year of "Out"

Some combo of going out more (leaving the house), and throwing stuff out in prep for the house move. 

[[Theme]]

[[Chatty G q's on 2026 theme]] - Some of this is irrelevant, but I shall go through it to prompt some thinking. 

## Review of Last Year.

I was pretty inconsistent on last year's. Although I did get eventually have an eye test, and visit the dentist. I do need to go back to the dentist for a filling/extraction they suggested. 

Right now, I think the template I came up with last year still applies, but can review that during the process. [[Templates/Daily Note template]]

Recent weeks have seen an improvement in the grooming etc though, and self maintenance. 

It could be argued I've improved on the [[Theme#INTEGRITY/FOLLOW UP/FOLLOW THROUGH]] too. Though that'd mostly be a side effect of having not really done much. Easy to increase the average if the sample size is small. So for this year, it would be good to do more, but also agree to/be open to accepting more. 

## Clarifying 2026

### The Year of Out.

- Get out more
		- Bikes
		- Gigs
		- "Experiences"
		- Seeing friends/family
- Clear out
		- The house
		- My brain
		- Lists
		- Get a laugh "out" of Shiv, every day

So those are the two sub-themes I reckon. 

Just added "What have I done today that helps get *out*?" to my daily note template. Accidentally that's pretty good. It could pertain to actually doing something directly aligned to the theme of Out, but also to things that clear the way for doing that. Agreeing to/arranging something for a future "out" would count as a contribution. 

I also liked chatty g's idea that "If it required shoes, it counts."

"- "One deliberate Out session per week" (bigger, planned)" sounds good too. During the colder months, that could even count as taking the dogs for a walk. 

### "Out" Menu

**Important**- any one of these counts. The menu can change as time goes on.

OUT (Experiences)

- Go for a walk
- Ride my bike
- See one person IRL
- Attend something slightly inconvenient
- Maintain correspondence/comms

OUT (Objects)

- Remove 5 items from one drawer/other place
- Identify one "why do I still own this?"
- Make a donation bag
	- Get rid of a donation bag
- Photograph items to sell (even if you don't list them yet)

> Every item I remove is one less decision Future Me has to make.

## Monthly Reflection Questions

Once a month, answer these briefly (bullet points only):

1. Where did I feel most "out" this month?
2. What made going out easier than expected?
3. What resisted being thrown out — and why?
4. Did anything I removed create unexpected relief?
5. Is "out" currently meaning movement or lightness more?

## Mid-decision Questions:

- "Does (doing) this move me out or keep me in?"
- "Will this come with me?"
- "Shoes or sofa?"

***"If in doubt, out."***

### Further Notes

Seems like a reasonable foundation. January may be rough, with the impending funeral, and money being tight (thanks, payday schedule). My concern is it will feel like most of January is just *waiting for the funeral*, then a particularly tough day or two, though hopefully those days will also bring a sense of relief and/or closure. 



# Daily Note Template

Remember the ~~Cant~~ [[Theme for 2026]]

***"If in doubt, out."***

What have I done today that helps get *out*?
For quick wins, see [[Theme for 2026#"Out" Menu]]

## Thing That Must Get Done Today

## What's On My Mind

## (Specific) Thing(™) That I'm Grateful For

## Thing That Went Well

## Notes for the Day
